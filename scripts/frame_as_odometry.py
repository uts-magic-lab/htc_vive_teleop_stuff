...
"""
Publish a frame 3D pose as a Odometry message continuously.

Author: Sammy Pfeiffer <Sammy.Pfeiffer at student.uts.edu.au>
"""


class PublishFrameAsOdometry(object):
    def __init__(self, frame_to_posestamped,
        ...
        self.odom_pub = rospy.Publisher(topic_name + '_as_odometry',
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            o = Odometry()
            o.header.stamp = tfed_ps.header.stamp
            o.child_frame_id = self.frame_to_posestamped
            o.header.frame_id = self.reference_frame
            o.pose.pose = tfed_ps.pose
            # Maybe someone needs something smarter than this?
...
if __name__ == '__main__':
    rospy.init_node('frame_to_odometry')
    argv = rospy.myargv(sys.argv)
    if len(argv) < 3:
        print("Usage:")
        print(argv[0] + " frame_to_odometry reference_frame [rate]")
        exit(0)
    frame_to_odometry = argv[1]
    reference_frame = argv[2]
    if len(argv) == 4:
        rate = int(argv[3])
    else:
        rate = 10
    pfaodom = PublishFrameAsOdometry(frame_to_odometry,
                                     reference_frame,
                                     rate,
                                     verbose=False)
    pfaodom.run()
